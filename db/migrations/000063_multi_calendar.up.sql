...
-- 1. Drop the FK constraint that depends on the unique index.
ALTER TABLE calendars DROP FOREIGN KEY fk_calendars_campaign;

-- 2. Drop the UNIQUE constraint that limited to one calendar per campaign.
ALTER TABLE calendars DROP INDEX idx_calendars_campaign;

-- 3. Add a regular (non-unique) index for campaign lookups and re-add the FK.
...
-- 4. Add ordering and default flag columns.
...
-- 5. Mark all existing calendars as the default for their campaign.
UPDATE calendars SET is_default = 1;

-- 6. Add calendar_id FK to sessions for explicit calendar association.
...
-- 7. Backfill: link existing sessions to their campaign's single calendar.
UPDATE sessions s
